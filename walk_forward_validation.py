...
def walk_forward_validation(data_path="data/raw/eurusd_m1.csv"):
    """
    ...
    # Convert timestamp to datetime for easier splitting
    ...
    # Split into 4 weeks (approximately)
    week_duration = total_days // 4

    logger.info(f"\nSplitting into ~{week_duration}-day periods...")

    # Create splits
    splits = []
    for i in range(1, 4):  # 3 test periods (weeks 2, 3, 4)
        train_end = start_date + timedelta(days=week_duration * i)
        test_end = start_date + timedelta(days=week_duration * (i + 1))
        ...
        if len(train_df) > 100 and len(test_df) > 50:  # Minimum samples
            splits.append({
                'fold': i,
    ...
    for split in splits:
        ...
        # High-confidence signals (>0.65 threshold)
    ...
    worst_fold = results_df.loc[results_df['f1'].idxmin(), 'fold']

    logger.info(f"\nBest period:  Fold {best_fold} (F1: {results_df.loc[results_df['fold']==best_fold, 'f1'].values[0]:.4f})")
    logger.info(f"Worst period: Fold {worst_fold} (F1: {results_df.loc[results_df['fold']==worst_fold, 'f1'].values[0]:.4f})")
