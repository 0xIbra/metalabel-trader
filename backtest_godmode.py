...
def backtest_godmode_strategy(
    ...
    for step in timeline:
        ...
        if date_str != current_day:
            # Check Daily Loss for previous day
            if current_day is not None:
                daily_pnl = balance - daily_starting_equity
                daily_loss_pct = (daily_starting_equity - balance) / daily_starting_equity * 100
        ...
        # Track Trading Days (if a trade was opened or closed, strictly speaking, but we'll count active days)
        # Prop firms usually count days where a trade was opened.
        # For simplicity, we'll add to set if we take an action.

        # Track High Water Mark & Drawdown
        if balance > high_water_mark:
            high_water_mark = balance

        dd = (high_water_mark - balance) / high_water_mark * 100
    ...
    # Print Results
    logger.info("\n" + "=" * 80)
    logger.info("PROP FIRM CHECKLIST")
    logger.info("=" * 80)

    total_return_pct = ((balance - initial_balance) / initial_balance) * 100
    logger.info(f"1. Profit Target (10%): {total_return_pct:.2f}% [{'✅ PASS' if total_return_pct >= 10 else '❌ FAIL'}]")
    logger.info(f"2. Min Trading Days (5): {len(trading_days)} [{'✅ PASS' if len(trading_days) >= 5 else '❌ FAIL'}]")
    logger.info(f"3. Max Daily Loss Limit (5%): {max_daily_loss_pct:.2f}% [{'✅ PASS' if max_daily_loss_pct < 5 else '❌ FAIL'}]")
    logger.info(f"4. Max Overall Drawdown (6%): {max_drawdown_pct:.2f}% [{'✅ PASS' if max_drawdown_pct < 6 else '❌ FAIL'}]")
